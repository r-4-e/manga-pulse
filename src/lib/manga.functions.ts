...
// MangaDex public API base. No key required.
...
async function fetchCoverUrl(mangaId: string, fileName: string | null): Promise<string | null> {
...
async function cacheManga(items: MangaSummary[]) {
  ...
  await supabaseAdmin.from("manga_cache").upsert(
    items.map((m) => ({
      id: m.id,
      source: "mangadex",
      title: m.title,
...
export const searchManga = createServerFn({ method: "GET" })
  .inputValidator((d) => z.object({ q: z.string().min(1).max(120) }).parse(d))
  .handler(async ({ data }): Promise<MangaSummary[]> => {
    const url = new URL(`${MD}/manga`);
    url.searchParams.set("title", data.q);
    ...
    cacheManga(items).catch((e) => console.error("cache failed", e));
    return items;
  });
...
export const getManga = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<MangaSummary | null> => {
    // Try cache first
    const { data: cached } = await supabaseAdmin
      ...
      .eq("id", data.id)
      .maybeSingle();
    if (cached && Date.now() - new Date(cached.fetched_at).getTime() < 1000 * 60 * 60 * 24) {
      return {
        id: cached.id,
        title: cached.title,
    ...
    const url = new URL(`${MD}/manga/${data.id}`);
    url.searchParams.append("includes[]", "cover_art");
    url.searchParams.append("includes[]", "author");
    const res = await fetch(url);
