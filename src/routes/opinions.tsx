...
import { listOpinions, createOpinion } from "@/lib/opinions.functions";
...
export const Route = createFileRoute("/opinions")({
  ...
  head: () => ({
    meta: [
      { title: "Opinion Arena — Mangaverse" },
      { name: "description", content: "Drop hot takes. Real users vote. Defend your opinion or get demolished." },
    ],
  }),
  component: Opinions,
});
...
function Opinions() {
  ...
  const list = useServerFn(listOpinions);
  const create = useServerFn(createOpinion);
  const qc = useQueryClient();
  const { user } = useAuth();

  const { data, isLoading } = useQuery({
    queryKey: ["opinions", sort],
    queryFn: () => list({ data: { sort } }),
  });
    ...
    <main className="mx-auto max-w-2xl px-4 py-8">
      <header className="mb-6 text-center">
        ...
        <h1 className="font-display text-3xl font-bold sm:text-4xl">
          Drop a <span className="text-gradient">hot take</span>
        </h1>
      ...
      <section className="mb-6 rounded-2xl glass p-4">
        ...
        <input
          ...
          maxLength={140}
          placeholder={'e.g. "Marineford is overrated"'}
        ...
        <textarea
          ...
          maxLength={2000}
          rows={2}
          placeholder="Defend your take (optional)"
        ...
        <div className="mt-3 flex items-center justify-between">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs transition-colors ${
              anon ? "border-primary bg-primary/10 text-primary" : "border-white/10 text-muted-foreground"
            }`}
          >
          ...
          <button
            onClick={submit}
            disabled={busy}
            className="inline-flex items-center gap-1.5 rounded-full bg-[var(--gradient-primary)] px-4 py-1.5 text-sm font-semibold text-background glow-magenta disabled:opacity-50"
          >
      ...
      <div className="mb-4 flex gap-1 rounded-full glass p-1">
        {(["hot", "new", "controversial"] as const).map((s) => (
          <button
            key={s}
            onClick={() => navigate({ to: "/opinions", search: { ...Route.useSearch(), sort: s } })}
            className={`flex-1 rounded-full px-3 py-1.5 text-xs font-semibold capitalize transition-colors ${
              sort === s ? "bg-[var(--gradient-primary)] text-background" : "text-muted-foreground"
            }`}
          >
