...
import { getFeaturedManga } from "@/lib/manga.functions";
    ...
    <main>
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-12 pt-10 sm:pt-16">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-xs text-muted-foreground">
              ...
              Beta · join the first 1,000 critics
            </div>
          ...
          <div className="mx-auto mt-12 grid max-w-4xl grid-cols-1 gap-3 sm:grid-cols-3">
            <Feature icon={<Flame className="h-4 w-4" />} title="Tier rankings" body="Drag arcs into S–F. Auto-save. Share anywhere." />
            <Feature icon={<Swords className="h-4 w-4" />} title="Opinion arena" body="Drop hot takes. Real users vote." />
            <Feature icon={<TrendingUp className="h-4 w-4" />} title="Manga DNA" body="Your taste, distilled into a card you can flex." />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12">
        <div className="mb-6 flex items-end justify-between">
          ...
          <Link to="/search" className="text-sm font-medium text-foreground hover:underline">
            Browse all →
          </Link>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Answer 5 quick questions to unlock rankings, compatibility & a shareable card.
            </p>
          </div>
        </div>
        <Link
          to="/onboarding"
          className="inline-flex items-center justify-center gap-1.5 rounded-full bg-foreground px-4 py-2 text-sm font-semibold text-background hover:opacity-90"
        >
...
function Feature({ icon, title, body }: { icon: React.ReactNode; title: string; body: string }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-5 text-left">
      <div className="mb-3 grid h-8 w-8 place-items-center rounded-lg bg-muted text-foreground">
        {icon}
      </div>
