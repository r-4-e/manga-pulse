...
import { searchManga, getPopularManga } from "@/lib/manga.functions";
import { MangaCard } from "@/components/MangaCard";

export const Route = createFileRoute("/search")({
  head: () => ({
    meta: [
      { title: "Search manga — Mangaverse" },
      { name: "description", content: "Search across thousands of manga to rank, review, and debate." },
    ],
  }),
...
function SearchPage() {
  ...
  const popular = useServerFn(getPopularManga);

  const popularQ = useQuery({ queryKey: ["manga", "popular"], queryFn: () => popular() });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="font-display text-3xl font-bold sm:text-4xl">
        <span className="text-gradient">Find manga</span>
      </h1>
      ...
      <div className="mt-6 flex items-center gap-2 rounded-full glass px-4 py-2.5">
        ...
        <input
          ...
          placeholder="e.g. Berserk, Vagabond, JJK..."
